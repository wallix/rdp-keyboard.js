...
const keycodeActionToSingleScancode = function(code) {
    switch (code)
    {
...
const keycodeToSingleScancode = function(code) {
    ...
    default: return keycodeActionToSingleScancode(code);
    }
};
...
const ShiftMod    = 0x001;
const AltGrMod    = 0x002;
const CapsLockMod = 0x004;
const CtrlMod     = 0x008;
const AltMod      = 0x010;
const OEM8Mod     = 0x020;
const KanaMod     = 0x040;
const KanaLockMod = 0x080;
const NumLockMod  = 0x100;

// extra flags
const RightShiftMod = 0x200;
const RightCtrlMod  = 0x400;
...
const scancodesForSynchronizedMods = function(syncFlags) {
    return [
        (LShiftSC | ((syncFlags & SyncFlags.ShiftLeft)    ? KeyAcquire : KeyRelease)),
        (RShiftSC | ((syncFlags & SyncFlags.ShiftRight)   ? KeyAcquire : KeyRelease)),
        (LCtrlSC  | ((syncFlags & SyncFlags.ControlLeft)  ? KeyAcquire : KeyRelease)),
        (RCtrlSC  | ((syncFlags & SyncFlags.ControlRight) ? KeyAcquire : KeyRelease)),
        (AltSC    | ((syncFlags & SyncFlags.AltLeft)      ? KeyAcquire : KeyRelease)),
        (AltGrSC  | ((syncFlags & SyncFlags.AltRight)     ? KeyAcquire : KeyRelease)),
        (LMetaSC  | ((syncFlags & SyncFlags.OSLeft)       ? KeyAcquire : KeyRelease)),
        (RMetaSC  | ((syncFlags & SyncFlags.OSRight)      ? KeyAcquire : KeyRelease)),
        // KanaSC = AltGrSC
        // (KanaSC   | ((syncFlags & SyncFlags.Kana)         ? KeyAcquire : KeyRelease)),
    ];
};
...
class ReversedKeymap
{
    ...
    //   - ctrl+alt = altgr when _ctrlAndAltIsAltGr
    //   - eom8 when _ctrlRightIsOem8 && (_modFlags & RightCtrlMod)
    // _modFlags: all controls

    constructor(reversedLayout) {
        this._modFlags = 0;
        this._virtualModFlags = 0;
        this._configCtrlAndAltIsAltGr = true;
    ...
    get ctrlAndAltIsAltGr() {
        return this._ctrlAndAltIsAltGr;
    }

    set ctrlAndAltIsAltGr(ctrlAndAltIsAltGr) {
        this._configCtrlAndAltIsAltGr = ctrlAndAltIsAltGr;
        this._ctrlAndAltIsAltGr = this._altRightIsAltGr && this._configCtrlAndAltIsAltGr;
    ...
    set layout(reversedLayout) {
        ...
        this._ctrlRightIsOem8 = reversedLayout.ctrlRightIsOem8;
        this._altRightIsAltGr = reversedLayout.altRightIsAltGr;
        this._ctrlAndAltIsAltGr = this._altRightIsAltGr && this._configCtrlAndAltIsAltGr;
        this._updateVirtualMod();
    }
    ...
    toScancodesAndFlags(key, code, flag) {
        ...
        if (dk) {
            // release key without accent
            if (flag === KeyRelease) {
                return this._scancodeByModsToScancodes(this._keymap[dk[dk.length - 1]], flag);
            }

            const accu = this._scancodeByModsToScancodes(this._accentKeymap[dk[0]], flag);
            // release dead key
            accu.push(accu[Math.floor(accu.length / 2)] ^ KeyRelease);

            let i = 1;
            for (; i < dk.length - 1; ++i) {
                const scancodes = this._scancodeByModsToScancodes(this._keymap[dk[i]], flag);
                accu.push(...scancodes);
                // release dead key
                accu.push(scancodes[Math.floor(scancodes.length / 2)] ^ KeyRelease);
            }
    ...
    _scancodeByModsToScancodes(scancodeByMods, flag) {
        ...
        // alt is maybe an altGr on client, but alt on server
        // relase alt before shift, because alt+shift is a shortcut for switch a layout
        if (!this._altRightIsAltGr && (this._modFlags & AltGrMod)) {
            accu.push(AltGrSC | release);
        }
        ...
        //         if ((this._modFlags & RightCtrlMod) && !this._ctrlRightIsOem8) accu.push(RCtrlSC | release);
        //     }
        // }
        ...
        if ((this._virtualModFlags ^ expectedModFlags) & AltGrMod) {
            if (this._ctrlAndAltIsAltGr) {
                if (expectedModFlags & AltGrMod) {
                    accu.push(AltGrSC | down);
                }
                // remove ctrl and alt because ctrl+alt = altgr
                else if (this._virtualModFlags & AltGrMod) {
                    if (this._modFlags & AltGrMod) {
                        accu.push(AltGrSC | release);
                        // if (this._modFlags & AltMod) {
                        //     accu.push(AltSC | release);
                        // }
                    }
                    else {
                        accu.push(AltSC | release);
                    ...
                    if (this._modFlags & CtrlMod) {
                        accu.push(LCtrlSC | release);
                    }
                    if (!this._ctrlRightIsOem8 && (this._modFlags & RightCtrlMod)) {
                        accu.push(RCtrlSC | release);
                    }
                }
            }
            else {
        ...
        // // KanaLockMod
        // const hasKanaLock = (this._virtualModFlags ^ expectedModFlags) & KanaLockMod;
        // if (hasKanaLock) {
        //     accu.push(KanaLockSC | down);
        //     accu.push(KanaLockSC | release);
        // }
        ...
        // reset emulated keys
        while (accuLen > 0) {
            accu.push(accu[--accuLen] ^ KeyRelease);
        }
    ...
    _modToScancode(code, flag) {
        switch (code) {
            ...
            // case "KanaLock":
            //     if (flag === KeyAcquire) {
            //         this._modFlags ^= KanaLockMod;
            //         this._virtualModFlags ^= KanaLockMod;
            //     }
            //     return [KanaLockSC | flag];
    ...
    _updateVirtualMod() {
        const ctrl = this._ctrlRightIsOem8 ? CtrlMod : (CtrlMod | RightCtrlMod);
        const alt = this._altRightIsAltGr ? AltMod : (AltMod | AltGrMod);
        const hasCtrl = this._modFlags & ctrl;
        const hasAlt = this._modFlags & alt;
        this._virtualModFlags = this._modFlags & (ShiftMod | CapsLockMod);
        this._virtualModFlags |= (this._modFlags & RightShiftMod) ? ShiftMod : 0;
        this._virtualModFlags |= (this._ctrlAndAltIsAltGr && hasCtrl && hasAlt) ? AltGrMod : 0;
        this._virtualModFlags |= (this._altRightIsAltGr && (this._modFlags & AltGrMod)) ? AltGrMod : 0;
        this._virtualModFlags |= (this._ctrlRightIsOem8 && (this._modFlags & RightCtrlMod)) ? OEM8Mod : 0;
    }
};

ReversedKeymap.version = "1.1.0";
...
try {
    ...
    module.exports.scancodesForKeyAcquireMods = scancodesForKeyAcquireMods;
    module.exports.scancodesReleaseAllMods = scancodesReleaseAllMods;
